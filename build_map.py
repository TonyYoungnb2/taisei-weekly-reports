...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  #legend .dot { width:10px; height:10px; display:inline-block; box-sizing:border-box; border:1.5px solid #fff; border-radius:50% 50% 50% 0; transform:rotate(-45deg); box-shadow:0 1px 2px rgba(0,0,0,.25); }
  #legend .d-done   { background:#0d9488; border-color:#fff; }
  #legend .d-undone { background:#7c3aed; border-color:#fff; }
...
// 水滴针尺寸随缩放放大：zoom 越大针越大（修复“放大后针变小”的观感）
function pinSize(zoom) {
  var s = 22 + (zoom - 5) * 2.0;            // 针高随缩放增大
  if (s < 18) s = 18;
  if (s > 56) s = 56;
  return s;
}
// 水滴针图标 HTML（teardrop：方形圆角旋转45°，底部尖端对准坐标）
...
PROJECTS.forEach(function(p) {
  ...
  markers.push({ dot: mk, news: p.news_count, r: s, p: p });
});

// 缩放时重算所有针大小，保持“放大→针变大”
function rescale() {
  var z = map.getZoom();
